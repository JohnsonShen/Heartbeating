...
#define PLLCTL_SETTING  CLK_PLLCTL_160MHz_HXT
...
void SYS_Init(void)
{
    ...
    SYS->GPB_MFPL |= (SYS_GPB_MFPL_PB5MFP_EADC0_CH5);
...
{	
	int i;
		...
		ADCData[1] = ADCData[0];
		ADCData[0] = g_u32ADCValue_ch10;
		//report sensor
		report_sensors((int16_t)g_u32ADCValue_ch10);
        ...
        if(((HeartFreq[0]-3) > HeartFreq[1]) && ((HeartFreq[2]-3) > HeartFreq[1]))
        {
          ...
          HeartFreq1[1] = HeartFreq1[1];//*(float)1.1;
...
void ADC00_IRQHandler(void)
{
		...
		g_u32ADCValue_ch10 = EADC_GET_CONV_DATA(EADC, 0);
...
int32_t main(void)
{
    ...
    /* Heart beating function test */
    HeartBeating_Function();
